...
logger.error("saveTransaction start");
		// 매입, 매도 체크
		...
		logger.error("saveTransaction middle");
		// 상품의 유효성 체크
		GoodsVO gv = (GoodsVO) this.selectDetail1(search);
		logger.error("saveTransaction this.selectDetail1");
		if(gv == null){
			logger.error("chk1");
		...
		logger.error("saveTransaction PaymentSearchVO search1 = new PaymentSearchVO()");
		// 중복구매 체크
