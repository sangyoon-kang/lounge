...
<div class="contract-header bg-dark-grey py-2 text-center column">
          <small class="upper letter-space bold "><span class="${vo.bsType eq 'B' ? 'text-red' : 'text-blue'}">fx</span>race</small>
          <h4 class="text-300">손익 분배 계약서 <small class="badge ${vo.bsType eq 'B' ? 'bg-red' : 'bg-blue'}">${vo.bsType eq 'B' ? '매수' : '매도'}</small></h4>
          <div class="flex offer justify-center">
               <span></span>
               <small class="upper">${vo.bsType eq 'B' ? 'buying offer' : 'selling offer'}</small>
               <span></span>
          </div>
</div>
<div class="contract-action  px-1 no-print ${vo.bsType eq 'S' ? 'bg-blue' : 'bg-red'}">
          <div class=" flex flex-wrap text-center align-center py-1 bg-transparent">

			  <a type="button" onclick="window.print()" class=" print" style="color: ${vo.bsType eq 'S' ? '#0066d9' : '#E3342F'}" name="button">
...
<div class="cw100">
		...
		<div class=" cttxt">
			...
			<div class="cttxt_info">
				...
				<p>문의전화<b >${tagoplusSolution1_session_site.customerCenter }</b></p>
			</div>
		</div>
		<div class="cform_wrap cw100">
		<h4><i class="fas fa-exclamation-circle"></i>&nbsp;&nbsp;본 상품은 고객님께서 <b>계약하신 상품의 금액</b>만 해당됩니다.</h4>
			<table width="100%">
				<thead>
				<tr>
					<th width="30%">상품구분</th>
				...
				<tr>
					<td class="px-1">
						<span class="${vo.bsType eq 'B' ? 'chart-green' : 'chart-red'} small">0.1랏 ${vo.bsType eq 'B' ? '매수' : '매도'}</span>
				...
				<tr>
					<td class="px-1">
						<span class="${vo.bsType eq 'B' ? 'chart-green' : 'chart-red'} small">0.2랏 ${vo.bsType eq 'B' ? '매수' : '매도'}</span>
						<p class="small">포지션 상품에서 발생될 ±손익에 대하여</p>
					</td>
					<td class="px-1">
				...
				<tr>
					<td class="px-1">
						<span class="${vo.bsType eq 'B' ? 'chart-green' : 'chart-red'} small">1랏 ${vo.bsType eq 'B' ? '매수' : '매도'}</span>
						<p class="small">포지션 상품에서 발생될 ±손익에 대하여</p>
					</td>
					<td class="px-1">
						<p class="text-left my-1 small">렌트보증금
				...
				<tr>
					<td class="px-1">
						<span class="${vo.bsType eq 'B' ? 'chart-green' : 'chart-red'} small">2랏 ${vo.bsType eq 'B' ? '매수' : '매도'}</span>
						<p class="small">포지션 상품에서 발생될 ±손익에 대하여</p>
					</td>
					<td class="px-1">
						<p class="text-left my-1 small">렌트보증금
