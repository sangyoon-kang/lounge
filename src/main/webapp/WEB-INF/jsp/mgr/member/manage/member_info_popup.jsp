...
<%--
 ...
 --%>
<style>
	#wrap_pop {float:left; width:750px !important; margin:20px; height: auto;}
	textarea[disabled], input[disabled] {background-color: #ffffff; resize: none}
</style>
	...
	<table class="board_write" id="write_member">
		...
		<c:if test="${search.mseq eq 0}">
			...
			<c:if test="${setting.recommendFlag > 0 }">
				<tr>
					...
					<td colspan="3">${vo.recommNickname }
						<span></span>
					</td>
				</tr>
			</c:if>
		...
		<c:if test="${setting.addrFlag > 0 }">
			<tr>
				...
				<td colspan="3">
					<c:if test="${vo.zipcode ne null and vo.zipcode ne ''}">
						(우 : ${vo.zipcode })
					</c:if>
					<c:if test="${vo.addr1 ne null and vo.addr1 ne ''}">
						&nbsp;${vo.addr1 }
					</c:if>
					<c:if test="${vo.addr2 ne null and vo.addr1 ne ''}">
						&nbsp;${vo.addr2 }
					</c:if>
				</td>
			</tr>
		...
		<tr>
			<th>관리자메모</th>
			<td colspan="3">
				<textarea name="memoAdmin" class="w100" rows="5" disabled>${vo.memoAdmin }</textarea>
			</td>
		</tr>
		<tr>
			...
			<td colspan="3">
				<c:forEach items="${bankList}" var="m" varStatus="s">
					<c:if test="${vo.bank eq m.codeName}">
						${m.codeName}
					</c:if>
				</c:forEach>
				<c:if test="${vo.bankAccount ne null and vo.bankAccount ne ''}">
					&nbsp;${vo.bankAccount }
				</c:if>
				<c:if test="${vo.accountOwner ne null and vo.accountOwner ne ''}">
					<br>예금주 : ${vo.accountOwner }
				</c:if>
			</td>
		</tr>
		<tr>
			<th>메일수신으로</th>
			<td colspan="3">
				<input type="checkbox" name="transEmail" value="Y" disabled>
