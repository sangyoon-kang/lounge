...
<c:forEach var="m" items="${list }" varStatus="s">
		...
		<c:if test="${m.gradeLevel ne 1}">						
						<a onclick="javascript:$.Nav('go', './member_list.do', {glevel: '${m.gradeLevel}'});" class="bt_copy">회원 보기</a>
		</c:if>
		<c:if test="${m.gradeLevel eq 1}">
						<a onclick="javascript:$.Nav('go', '${URL_ADMIN_ROOT }/base/basic/admin_list.do', null, null, true);" class="bt_copy">관리자 보기</a>
		</c:if>
					</td>
					<td>
		<c:if test="${m.gradeLevel ne 1}">					
